...
class WindowFactory {
    companion object {
        fun buildOnProgressDialog(title: String, msg: String): Stage {
            val stage = Stage()
            stage.scene = Scene(FXMLLoader.load<Parent>(ClassLoader.getSystemResource("layout/simpleProgressDialog.fxml")))
            stage.title = title
        ...
        fun ShowTestScene() {
            val stage = Stage()
            stage.scene = Scene(FXMLLoader.load<Parent>(ClassLoader.getSystemResource("layout/testScene.fxml")))
            stage.show()
        }
